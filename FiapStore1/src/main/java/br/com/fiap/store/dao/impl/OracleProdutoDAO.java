...
public class OracleProdutoDAO implements ProdutoDAO {
	...
	public Produto buscar(int id) {
		...
		try {
			...
			stmt = conexao.prepareStatement(
					"SELECT * FROM TB_PRODUTO INNER JOIN TB_CATEGORIA ON TB_PRODUTO.CD_CATEGORIA = TB_CATEGORIA.CD_CATEGORIA WHERE TB_PRODUTO.CD_PRODUTO = ?");
			stmt.setInt(1, id);
	...
	public List<Produto> listar() {
		...
		try {
			conexao = ConnectionManager.getInstance().getConnection();
			stmt = conexao.prepareStatement(
					"SELECT * FROM TB_PRODUTO INNER JOIN TB_CATEGORIA ON TB_PRODUTO.CD_CATEGORIA = TB_CATEGORIA.CD_CATEGORIA");
			rs = stmt.executeQuery();
